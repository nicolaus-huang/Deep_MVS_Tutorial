...
def filter_depth(args, scan: str = ""):
    ...
    def add_vertices(item):
        ref_view, src_views = item
    ...
    pool.map(add_vertices, pair_data)
...
if __name__ == "__main__":
    ...
    if input_args.output_type == "fusion" or input_args.output_type == "both":
        ...
        for input_scan in scans:
            args, scan = input_args, input_scan
            # the pair file
            pair_file = os.path.join(args.input_folder, scan, "pair.txt")
            # for the final point cloud
            vertices = []
            ...
            def add_vertices(ref_view, src_views, tree):
                # load the reference image
                ref_img, original_h, original_w = read_image(
                    os.path.join(args.input_folder, scan, "images/{:0>8}.jpg".format(ref_view)), args.image_max_dim)
                # load the camera parameters
                ...
                xyz_ref = np.matmul(np.linalg.inv(ref_intrinsics), np.vstack((x, y, np.ones_like(x))) * depth)
                xyz_world = np.matmul(np.linalg.inv(ref_extrinsics), np.vstack((xyz_ref, np.ones_like(x))))[:3]
                tree["1"].append(xyz_world.transpose((1, 0)))
                tree["2"].append((color * 255).astype(np.uint8))
            manager = Manager()
            sharedtree= manager.dict({"1":manager.list(),"2":manager.list()})
            pool = Pool(processes=args.num_process)
            pool.starmap(add_vertices, [(ref_view, src_views, sharedtree) for ref_view, src_views in pair_data])
            # print({k:list(v) for k,v in sharedtree.items()})
            vertices = sharedtree["1"]
            vertex_colors = sharedtree["2"]
            # print(len(vertices),vertices)
            # print(len(vertex_colors),vertex_colors)
            vertices = np.concatenate(vertices, axis=0)
            vertex_colors = np.concatenate(vertex_colors, axis=0)
            vertices = np.array([tuple(v) for v in vertices], dtype=[("x", "f4"), ("y", "f4"), ("z", "f4")])
            vertex_colors = np.array([tuple(v) for v in vertex_colors], dtype=[("red", "u1"), ("green", "u1"), ("blue", "u1")])
