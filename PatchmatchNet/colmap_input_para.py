...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_process", type=int, default=1, help="Running cpu num")
    ...
    def calc_score_para(i_index, j_index, tree, size):
        print("calc_score queue: " ,i_index ,j_index)
        s = calc_score(i_index, j_index)
        tree["1"][i_index*size + j_index] = s
        tree["1"][j_index*size + i_index] = s
    manager = Manager()
    temp_array = manager.list()
    for i in range(num_images):
        for j in range(num_images):
            temp_array.append(0)
    sharedtree= manager.dict({"1":temp_array})
    pool = Pool(processes=args.num_process)
    pool.starmap(calc_score_para, [(i,j,sharedtree,num_images) for i, j in queue])
    # print(sharedtree["1"])
    score = np.reshape(list(sharedtree["1"]),(num_images,num_images))
    # print(score[22,10]==score[10,22])
    # for i, j in queue:
    #     print("calc_score queue: " ,i ,j)
    #     s = calc_score(i, j)
    #     score[i, j] = s
    #     score[j, i] = s
    ...
    for i in range(num_images):
        print("wrtiting depth queue: %d"%i)
    ...
    for i in range(num_images):
        print("wrtiting images queue: %d"%i)
